...
To evaluate IZUMI without any friction, we have pre-configured fully active demo accounts loaded with real testnet transactions, live on-chain assets, and credit limits.

* **Live Frontend Web Application**: **`https://izumi-frontend.vercel.app`** *(Replace with your actual Vercel URL)*
* **Live Deployed API Base**: `https://izumi-production.up.railway.app`

### 🔑 Demo Account Credentials

#### **1. Saver Profile (High-Yield Portfolio)**
* **Email**: `isreaaldon@gmail.com`
* **Password**: *Not required in sandbox mode.* Just input the email on the Saver login screen and click **Login**.
* **What to review**:
  * View active yield positions (₦100 deposits auto-swapped for USD and locked in the 30-day high-yield vault on-chain).
  * Go to the **Deposit** page to copy the dedicated collections bank account details.
  * Go to the **Portfolio** page to track accrued DeFi yield.

#### **2. Borrower Profile (SME Credit & AI Underwriting)**
* **Email**: `israelore@gmail.com`
* **Password**: *Not required in sandbox mode.* Just input the email on the Borrower login screen and click **Login**.
* **What to review**:
  * View their credit health dashboard showing **"Izumi Bakery"** with an active credit limit, historical credit score **Grade B** generated by Gemini, and outstanding debt balances.
  * Go to **Izumi AI Advisor** and chat with the consultant (powered by a live **Gemini 2.5 Flash** model) about business cashflow optimization.
  * Adjust the **Split Repayment Sweep Dial** on the dashboard to configure automated terminal sweeps.

---

### 🧪 Sandbox Values for Signup Forms
If you want to register new profiles to test onboarding:
* **Saver BVN**: Any 11-digit number (e.g. `22233344455`). To trigger a live collections DVA check, use a real BVN (the system catches verification failures and falls back to a sandbox account cleanly).
* **Borrower BVN**: `12345678902`
* **Borrower CAC RC**: `RCCJND34` (or any string starting with `RC` / `BN`)
* **Borrower TIN**: `238U28U034` (must be at least 8 characters)
* *(Note: To test the compliance KYC/KYB error rejection states, enter any values starting with `999` to trigger an instant verification decline).*
